 ...
 */
export async function downloadUrl(url, destFile, options = {}) {
    let current = url;
    for (let hop = 0; hop <= MAX_REDIRECTS; hop += 1) {
        ...
        if (status >= 300 && status < 400 && response.headers.location !== undefined) {
            ...
            if (next === current)
                throw new Error(`skill-manager: redirect loop at ${current}`);
            current = next;
            continue;
        }
        if (status !== 200) {
        ...
        await new Promise((resolve, reject) => {
            ...
            response.on('error', (error) => fail(new Error(`skill-manager: download stream failed: ${error.message}`)));
            writer.on('error', (error) => fail(new Error(`skill-manager: cannot write ${path.basename(destFile)}: ${error.message}`)));
...
export async function downloadToTemp(url, tmpBase, options = {}) {
